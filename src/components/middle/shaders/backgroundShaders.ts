export const vertexShader = `
attribute vec4 a_position;

void main() {
  gl_Position = a_position;
}
`;

export const fragmentShader = `
precision highp float;

uniform vec2 resolution;

uniform vec3 color1;
uniform vec3 color2;
uniform vec3 color3;
uniform vec3 color4;

uniform vec2 color1Pos;
uniform vec2 color2Pos;
uniform vec2 color3Pos;
uniform vec2 color4Pos;

void main() {
  vec2 position = gl_FragCoord.xy / resolution.xy;
  position.y = 1.0 - position.y;

  float dp1 = distance(position, color1Pos);
  float dp2 = distance(position, color2Pos);
  float dp3 = distance(position, color3Pos);
  float dp4 = distance(position, color4Pos);
  float minD = min(dp1, min(dp2, min(dp3, dp4)));
  float p = 3.0;

  dp1 = pow(1.0 - (dp1 - minD), p);
  dp2 = pow(1.0 - (dp2 - minD), p);
  dp3 = pow(1.0 - (dp3 - minD), p);
  dp4 = pow(1.0 - (dp4 - minD), p);
  float dpt = abs(dp1 + dp2 + dp3 + dp4);

  gl_FragColor =
    (vec4(color1 / 255.0, 1.0) * dp1 / dpt) +
    (vec4(color2 / 255.0, 1.0) * dp2 / dpt) +
    (vec4(color3 / 255.0, 1.0) * dp3 / dpt) +
    (vec4(color4 / 255.0, 1.0) * dp4 / dpt);
}
`;

export function loadShaders(
  gl: WebGLRenderingContext,
  shaderSources: [vertexShader: string, fragmentShader: string],
): readonly [WebGLShader, WebGLShader] {
  const [vertexShaderSource, fragmentShaderSource] = shaderSources;
  return [
    loadShader(gl, vertexShaderSource, gl.VERTEX_SHADER),
    loadShader(gl, fragmentShaderSource, gl.FRAGMENT_SHADER),
  ] as const;
}

function loadShader(
  gl: WebGLRenderingContext,
  shaderSource: string,
  shaderType: number,
): WebGLShader {
  const shader = gl.createShader(shaderType)!;
  gl.shaderSource(shader, shaderSource);
  gl.compileShader(shader);
  gl.getShaderParameter(shader, gl.COMPILE_STATUS);
  return shader;
}
type HexString = `#${string}`;
export function hexToVec3(
  hex: HexString,
): readonly [r: number, g: number, b: number] {
  const colorIdentifier = hex.slice(1);

  const r = parseInt(colorIdentifier.slice(0, 2), 16);
  const g = parseInt(colorIdentifier.slice(2, 4), 16);
  const b = parseInt(colorIdentifier.slice(4, 6), 16);

  return [r, g, b] as const;
}

export type ThemeShaderConfig = {
  colors: {
    color1: HexString;
    color2: HexString;
    color3: HexString;
    color4: HexString;
  };
};

type ShadersConfig = {
  dark: ThemeShaderConfig;
  light: ThemeShaderConfig;
};

export const shadersConfig:ShadersConfig = {
  dark: {
    colors: {
      color1: '#fec496',
      color2: '#dd6cb9',
      color3: '#962fbf',
      color4: '#4f5bd5',
    },
  },
  light: {
    colors: {
      color1: '#dbddbb',
      color2: '#6ba587',
      color3: '#d5d88d',
      color4: '#88b884',
    },
  },
};
